...
int32_t main(int32_t argc, char **argv)
{
    ...
    for (uint32_t loop = 0; loop < totalLoops; ++loop) {
        ...
        if (NeedAivStore(opt)) {
            ret = RunProducerWithPoll(opt, buf, ProducerKind::AivStore, producerStream, aicpuStream, &aiv);
            if (ret != 0) return ret;
        }
        if (NeedSimtStore(opt)) {
            ret = RunProducerWithPoll(opt, buf, ProducerKind::SimtStore, producerStream, aicpuStream, &simtStore);
            if (ret != 0) return ret;
        }
